SELECT 
	...
	T4.[TaxID0],
	T2.[ShipDate],
	( SELECT TOP 1 (ISNULL((SELECT TOP 1 T4.NfmName from ONFM T4 WHERE T4.AbsEntry = T7.Model),'') + ' / ' + T7.SeriesStr + ' / ' + CAST(T7.SERIAL AS VARCHAR) ) as 'Documento' from INV1 T6 INNER JOIN OINV T7 ON (T7.DocEntry = T6.DocEntry) WHERE T6.PickIdNo = T0.AbsEntry  ) as DocEntryNF,	
	CASE T1.[PickStatus] 
